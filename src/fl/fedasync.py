...
class FedAsyncServer(BaseServer):
    ...
    def aggregate(self):
        ...
        trained_state, _, _, model_version = self.buffer[0]
        current_weights = self.global_model.state_dict()
        alpha_t = self.alpha * staleness(model_version)
        new_weights = {
            k: (1 - alpha_t) * current_weights[k].cpu() + alpha_t * trained_state[k]
...
class FedAsyncClient(BaseClient):
